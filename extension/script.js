...
function buildMessageHTML() {
  if (activeTmpl === 'vraag') {
    ...
    return `<div><b>• wachtrij:</b><br>${w || '…'}</div><br>` +
           `<div><b>• klantnummer:</b><br>${k || '…'}</div><br>` +
           `<div><b>• klantvraag:</b><br>${v || '…'}</div><br>` +
           `<div><b>• waar loop je vast:</b><br>${l || '…'}</div><br>` +
           `<div><b>• gewenste uitkomst:</b><br>${u || '…'}</div>`;
  } else {
    ...
    let msg = `<div><b>• antwoord:</b><br>${a || '…'}</div>`;
    if (b) msg += `<br><div><b>• bron:</b><br>${b}</div>`;
    if (v) msg += `<br><div><b>• vervolgstap:</b><br>${v}</div>`;
    return msg;
  }
}
